...
if Code.ensure_loaded?(:yamerl) do
  defmodule Mix.Tasks.Sql.Gen.Test do
    ...
    def run([base]) do
      create_file("test/conformance/e_test.exs", test_template([mod: SQL.Conformance.ETest, dir: Path.join(base, "E")]), force: true)
      create_file("test/conformance/f_test.exs", test_template([mod: SQL.Conformance.FTest, dir: Path.join(base, "F")]), force: true)
      create_file("test/conformance/s_test.exs", test_template([mod: SQL.Conformance.STest, dir: Path.join(base, "S")]), force: true)
      create_file("test/conformance/t_test.exs", test_template([mod: SQL.Conformance.TTest, dir: Path.join(base, "T")]), force: true)
    end
    ...
    defmodule <%= inspect @mod %>.Adapter do
      ...
      def to_iodata(value, _context, _indent) when is_atom(value) do
        String.upcase(Atom.to_string(value))
      end
      def to_iodata(token, context, indent), do: SQL.Adapters.ANSI.to_iodata(token, context, indent)
      def to_iodata(tokens, context, indent, acc), do: SQL.Adapters.ANSI.to_iodata(tokens, context, indent, acc)
    end
